...
public class ApiEx2 {
    public static void main(String[] args) {
        /*
        java.util 패키지 : java.lang 패키지의 추가 기능판으로
                         대부분 import가 필요한 클래스로 구성됨. */
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일 hh시 mm분 ss초"); //pattern:형식
        //String today1 = sdf.format(strNow); //자바8 미만
        String today2 = sdf.format(now);    //자바8 이상부터 문자로 굳이 안바꿔도 가능하게 됨.
        //System.out.println("문자열에 표시형식 날짜로 표시 : "+today1);
        System.out.println("날짜에 표시형식 날짜로 표시 : "+today2);

        int y = now.getYear() + 1900;
        int M = now.getMonth() + 1; //0부터 시작하기 때문에 1 더해주기
        int d = now.getDate();
        int w = now.getDay(); //요일
        int h = now.getHours();
        ...
        // ->자바8 이상은 추상체와 정적 구조로 구성되어 있으므로 별도의 객체를 생성하지 않아도 됨.
        Calendar n = Calendar.getInstance(); //1.7에서 이렇게 작성함.

        //자바 8 이상은 LocaleDate을 활용하여 시간대나 지역의 지역성을 나타냄.
    }
}
